...
and is commonly typified as the ID of the cloud instance, but can be a unique
string.
...
- The Rackspace Monitoring agent is not installed on your server. For installation instructions, see
    [Install and configure the Rackspace Monitoring Agent](/support/how-to/install-and-configure-the-rackspace-monitoring-agent).
- The agent is installed but is not connected to your server. For the agent to
    work correctly, it must be associated with your server by using a Rackspace
    Monitoring entity. The entity is a representation of your server in the
    monitoring system.
- The agent is installed but is not started or is not configured to start
...
3. Check for errors and status information in the
    **/var/log/rackspace-monitoring-agent.log** file:
    ...
    Look for errors, like the following excerpt from a log file demonstrates a
...
4. Run the following command to verify that the agent is running:
    ...
    Start the service if the status is not shown as running:

        $ service rackspace-monitoring-agent start

5. Make sure that you have firewall rules that allow IPv6 communication. If you
    have IPv6 disabled, make sure that the firewall allows agent
    communication.

#### Windows

1. Verify that the Rackspace monitoring agent exists under **C:\\Program
    Files\\Rackspace Monitoring\\rackspace-monitoring-agent.exe**. 

    **Note**:If you use a 32-bit version on a 64-bit Windows computer, the
    monitoring agent is installed under **C:\\Program Files (x86)\\Rackspace
...
2. Verify that the **C:\\ProgramData\\Rackspace
    ...
    contains a valid agent token.
3. Check for errors and status information in the **C:\\ProgramData\\Rackspace
    Monitoring\\log.txt** file.
4. Verify that the monitoring agent is running by opening the Task Manager,
    ...
    agent service.
5. Make sure that you have firewall rules that allow IPv6 communication. If you
    have IPv6 disabled, make sure that the firewall is permitting the agent to
    communicate.
...
To troubleshoot a monitoring agent by using Raxmon, complete the following
steps:
...
4. Examine the output to see if it has two entites as shown in the example
    below:
...
3. If the call is successful, you will receive a list of entities and their
