...
a small geographical area.  Rackspace leaders identified which data
centers offer the most reliability, high availability, and efficiency while
offering our customers access to the full range of Rackspace products and
services.

Because of previous outages at the SMDC facility, as well as limited capacity
and aging infrastructure, Rackspace plans to exiting the SMDC facility. We will
...
Located in Ashburn, VA, the IAD3 data center offers a more robust networking
and electrical infrastructure and consumes 5.525 MW of power with capacity for
an additional 3.143 MW. It also provides our entire suite of products,
including RackConnect Global, which enables you to connect your Rackspace
...
scheduled for a date in April through September 2020. The move groups are
scheduled to run in a maintenance window starting on either a Friday or Saturday
...
offline while it is moved to the new location.
...
-	At the start of the maintenance window on the night of the migration,
   ...
   shutdowns, see the
   **My environment is sensitive to the way it is handled during a shut down. Can you help?**
   section.
...
-	Perform the migration tasks, including stop replication jobs, present data
   to the target host, and so on.
-	Perform post-migration steps including map disks to VMs, take a snapshot
   (VMWare&reg;), add the Servicenet interface, power on devices, and so on.
...
-	For any colocation devices and dedicated servers that we cannot virtualize,
   ...
   The customer could send us a new device, Rackspace could procure a new
   device, or the customer could accept downtime of up to 48 hours while we
...
These IP changes include, but are not limited to, private and third-party VPN
tunnel peer IPs, public-facing servers, and load balancer virtual IP addresses
(VIPs) with public IPs, whitelists, and third party network gear. Before the
migration, Rackspace will provide documented information of the previous IP
...
addresses, such as  VPN tunnels, DNS, application interface whitelisting, and
so on.
...
we can also agree the process for this in advance of the move.
...
relating to the new data center that your telecoms provider requires.
...
The team will work with you and your Customer Success Team team to determine and
agree to any special requirements that you might have for pre-migration and
