...
Cuurently, OpenSSL supports the following EC algorithms:
...
You might also encounter Public Key Crytography Standard 8 (PKCS8) format private keys in PEM files, which looks similar to the following example:
...
openssl cipher name (see Manual:enc(1) for a list of valid cipher names).

To convert a PKCS8 file to a traditional unencrypted EC format,  drop the first argument:
...
key file to an ecrypted PKCS8 format DER encoded key:
...
To generate a private/public key pair from a pre-eixsting parameters file use
the following command:
