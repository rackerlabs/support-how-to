...
This articles serves as a guide for most up-to-date disconnect codes and reasons.

### Information:

Please note that some of the disonnect codes returned are extended codes. If you cannot find the disconnect code in the Primary Disconnect Reasons list, see the Extended Reasons list. When troubleshooting client-side connections be aware that dependong on configurations such as security level and encryption, the client may attempt connection multiple times and disconnect before the final connection is made.

### Script

The following script is a PowerShell command to query COM MSTscAx with a decimal disconnect code ID. If any code is found the description will be turned.
...
Note: If recieving "an internal error has occurred." this is the result when disconnect code is not found and return description is not valid.

### Example

Below is an example event log entry event ID 1026 of an RDP client session disconnected event (event code 263 means no error.)
