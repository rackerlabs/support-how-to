...
Based on the size and configuration of your existing cloud instance, select an equivalent size and configuration for the Rackspace Cloud Servers instance. Consider any resources that you want to migrate from your current hosting provider such as application, and database resources.
...
Select an equivalent instance type to that of you current server and operating system, and then [provision a new instance](/how-to/create-a-cloud-server) on Rackspace Cloud Servers.
...
Public Cloud by using [InstanceSync](https://github.com/cloudnull/InstanceSync/).
   ...
   Note: InstanceSync is not officially supported by Rackspace, and we cannot 
...
2. Create a new destination server in the Rackspace Public Cloud and record
   ...
   operating system (OS), such as Ubuntu&reg; 14, contact Rackspace Support for assitance building the destination server.
